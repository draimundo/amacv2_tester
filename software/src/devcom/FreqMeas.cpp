...
void FreqMeas::reset(bool active){
    uint32_t data = m_dev->read_reg(4*3*m_id);
...
void FreqMeas::set_ts_cnt(uint32_t ts_cnt){
    ...
    m_dev->write_reg((4*3*m_id), data);
}

void FreqMeas::read(){
  uint32_t edge_data = m_dev->read_reg(4*(3*m_id+1));
  ...
  uint32_t dc_data = m_dev->read_reg(4*(3*m_id+2));
